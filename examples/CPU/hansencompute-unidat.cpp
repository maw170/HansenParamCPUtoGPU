...
int main(int argc, char *argv[]){
	...
	for(int i = 1; i <= njobs; i++){
		...
		filename << directory.str() << "/energy.out";
		dumstr = filename.str();
		const char* foochar1 = dumstr.c_str();
		cell.open(foochar1); //open the two input files to be used

		filename.str("");
		filename << directory.str() << "/intmolec.dat";
		dumstr = filename.str();
		const char* foochar2 = dumstr.c_str();
		mole.open(foochar2); //open the two input files to be used
